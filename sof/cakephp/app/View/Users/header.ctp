...
<body>
	...
	<div id="search_bar">
        ...
        <div id="right_side">
            <?php
					echo '<p>'.$this->Html->link('Solo en stock', array('controller' => 'products', 'action' => 'filterStock', 'full_base' => true))."&nbsp&nbsp&nbsp".$this->Html->link('Mi carrito',array('controller' =>'products','action'=>'carrito')).": ".$this->Carrito->calcularCarrito($this->Session->read('Cart'),$this->Session->read('CartQty'),$this->Session->read('CartPrc'))."$&nbsp&nbsp&nbsp".$this->Html->link('Mi Wish List',array('controller' =>'productwishlist','action'=>'index'))."&nbsp&nbsp&nbsp";                    if($this->Session->read('Auth.User.username')==null)
                    {
